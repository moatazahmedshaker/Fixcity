...
@app.route('/api/mobile/problems', methods=['POST'])
def mobile_report_problem():
    """Mobile app reports a problem"""
    data = request.get_json()
    
    # Generate tracking code (reuse your existing function)
    code = random_code(10)
    while Problem.query.filter_by(tracking_code=code).first():
        code = random_code(10)
    ...
    problem = Problem(
        tracking_code=code,
        title=data.get('title', 'Mobile Report'),
...
@app.route('/api/mobile/problems/<tracking_code>')
def mobile_track_problem(tracking_code):
    """Mobile app tracks a problem"""
    problem = Problem.query.filter_by(tracking_code=tracking_code).first()
...
@app.route('/api/problems/<int:problem_id>', methods=['GET', 'PUT'])
@app.route('/api/categories', methods=['GET'])
@app.route('/api/track/<code>', methods=['GET'])
